...
#with open(r"C:\Users\emili\OneDrive - Universidad Autonoma de Nuevo León\FCFM\Inteligencia Artificial\instancias\f_1_2.csv", "r") as file:
with open(r"C:\Users\DELL\Downloads\instancias\instancias\f_1_0.csv", "r") as file:
...
while terminar(sub1,sub2,sub3,sub4): 
    bestrow = [0, 0, 0, 0, 16, 0, 0, 0, 0, 0] #la mejor duracion de una act es de 15
    #se revisa todas las lineas para ver cual es la mejor (menor duracion)
    for i in rows:
        if int(i[4]) < int(bestrow[4]): #cada actividad se compara con la mejor actual
            bestrow = i #la mejor se guarda
    requerimentos(bestrow) #se revisan los requerimentos de la mejor
    inornot(bestrow) #Se revisa si se puede agregar al tema correspondiente 
    rows.remove(bestrow) #se borra de la lista principal